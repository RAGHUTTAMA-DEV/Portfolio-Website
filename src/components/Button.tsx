...
import type { Variant,IProject } from '../types/index';
...
type Props = {
  ...
  className?: string;
  variant?: Variant;
} & (ComponentPropsWithoutRef<typeof Link> | ButtonProps);
...
const Button = ({
  ...
  if (as === 'link') {
    const props = rest as ComponentPropsWithoutRef<typeof Link>;
    const { to, target } = props;

    if (target === '_blank') {
      return (
        <a className={buttonClasses} {...props} href={typeof to === 'string' ? to : '#'}>
          {variant !== 'link' && (
